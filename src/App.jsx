  ...
  return (
    ...
    >
      <style>{`
        ...
        body { margin: 0; background: #02000a; overflow-x: hidden; overflow-y: auto; }
      ...
      <div ref={cursorRef} className="pointer-events-none fixed left-0 top-0 z-[9999] hidden h-5 w-5 rounded-full border border-purple-100/80 bg-white/20 shadow-[0_0_25px_rgba(216,180,254,.95)] backdrop-blur-sm md:block" />
      <div ref={cursorGlowRef} className="pointer-events-none fixed left-0 top-0 z-[9998] hidden h-24 w-24 rounded-full bg-purple-400/20 blur-xl md:block" />
      ...
      {mist.map((item, i) => (
        <span key={`mist-${i}`} className="fixed h-32 w-[680px] rounded-full bg-gradient-to-r from-transparent via-purple-400/20 to-transparent blur-2xl" style={item} />
      ))}
      {particles.map((item, i) => (
        <span key={`particle-${i}`} className="fixed rounded-full bg-purple-100 shadow-[0_0_18px_rgba(216,180,254,.95)]" style={item} />
      ))}
      {comets.map((item, i) => (
        <span key={`comet-${i}`} className="fixed h-px w-32 bg-gradient-to-r from-transparent via-purple-100 to-fuchsia-200 shadow-[0_0_22px_rgba(216,180,254,.95)]" style={item} />
      ))}
      ...
      <header className="fixed left-0 right-0 top-0 z-50 px-5 py-5">
        <nav className="mx-auto flex max-w-7xl items-center justify-between rounded-full border border-purple-200/15 bg-[#050014]/55 px-5 py-3 backdrop-blur-2xl" style={{ animation: "navGlow 5s ease-in-out infinite" }}>
          ...
          <button onClick={() => setMenuOpen(!menuOpen)} className="md:hidden rounded-full border border-purple-200/15 px-4 py-2 text-sm">Menu</button>
        </nav>
        {menuOpen && (
      ...
      <section id="home" className="relative z-20 mx-auto flex min-h-screen w-full max-w-7xl items-center px-5 pb-20 pt-32">
        <div className="w-full max-w-5xl">
          <div className="hero-enter">
            <p className="font-cinzel text-sm tracking-[10px] text-purple-100/80">CREATED BY MAHMUDUL HASAN</p>
          ...
          <div className="glass-card rounded-[38px] p-7 md:p-10">
            <div className="mb-7 flex flex-wrap gap-3">
              {Object.entries(tabs).map(([key, tab]) => (
                <button key={key} onClick={() => setActiveTab(key)} className={`rounded-full px-5 py-3 text-sm font-bold tracking-[2px] transition ${activeTab === key ? "bg-purple-50 text-[#130020]" : "border border-purple-100/15 bg-purple-100/8 text-purple-50/70 hover:text-white"}`}>
                  {tab.label}
                </button>
              ))}
            </div>
        ...
        <div className="mt-14 grid gap-7 lg:grid-cols-3">
          {services.map((service, i) => (
            <div key={service.title} className="glass-card scan-card rounded-[36px] p-8 transition duration-500 hover:-translate-y-4" style={{ animation: `cardFloat ${6 + i * 0.5}s ease-in-out infinite` }}>
              <div className="mb-7 flex h-16 w-16 items-center justify-center rounded-2xl border border-purple-100/15 bg-purple-100/10 text-4xl text-purple-100 shadow-[0_0_35px_rgba(168,85,247,.3)]">{service.icon}</div>
        ...
        <div className="glass-card rounded-[42px] p-8 md:p-12">
          <div className="grid gap-8 lg:grid-cols-[.7fr_1.3fr] items-center">
            ...
            <div className="flex flex-wrap gap-4">
              {skills.map((skill) => (
                <span key={skill} className="rounded-full border border-purple-100/15 bg-purple-100/8 px-5 py-3 text-purple-50/75 backdrop-blur-xl transition hover:scale-105 hover:bg-purple-100/15">{skill}</span>
              ))}
            </div>
          </div>
        </div>
        ...
        <div className="grid gap-8 lg:grid-cols-[0.9fr_1.1fr]">
          <div className="premium-border rounded-[46px]">
            <div className="glass-card rounded-[45px] p-8 md:p-12">
              ...
              <h2 className="mt-5 font-cinzel text-5xl font-black leading-tight md:text-7xl glow-text">Let’s create your next website.</h2>
              <p className="mt-6 text-purple-50/65 leading-8">Want a portfolio, landing page, animated greeting page, or premium personal website? Send me a message and tell me what you want to build.</p>
              <div className="mt-8 grid gap-4 sm:grid-cols-2">
                <a href="mailto:hasanhridoy5191@gmail.com" className="group rounded-3xl border border-purple-100/15 bg-purple-100/8 p-5 text-left transition hover:-translate-y-2 hover:bg-purple-100/14"><p className="text-3xl">✉</p><h3 className="mt-4 text-xl font-black">Email</h3><p className="mt-2 break-all text-sm text-purple-50/60">hasanhridoy5191@gmail.com</p></a>
                <a href="https://www.tiktok.com/@hasanhridoy5191" target="_blank" rel="noreferrer" className="group rounded-3xl border border-purple-100/15 bg-purple-100/8 p-5 text-left transition hover:-translate-y-2 hover:bg-purple-100/14"><p className="text-3xl">♪</p><h3 className="mt-4 text-xl font-black">TikTok</h3><p className="mt-2 text-sm text-purple-50/60">@hasanhridoy5191</p></a>
                <a href="https://www.facebook.com/hasanhridoy5191" target="_blank" rel="noreferrer" className="group rounded-3xl border border-purple-100/15 bg-purple-100/8 p-5 text-left transition hover:-translate-y-2 hover:bg-purple-100/14"><p className="text-3xl">f</p><h3 className="mt-4 text-xl font-black">Facebook</h3><p className="mt-2 text-sm text-purple-50/60">@hasanhridoy5191</p></a>
                <a href="https://www.instagram.com/hasanhridoy5191" target="_blank" rel="noreferrer" className="group rounded-3xl border border-purple-100/15 bg-purple-100/8 p-5 text-left transition hover:-translate-y-2 hover:bg-purple-100/14"><p className="text-3xl">◎</p><h3 className="mt-4 text-xl font-black">Instagram</h3><p className="mt-2 text-sm text-purple-50/60">@hasanhridoy5191</p></a>
                <a href="https://wa.me/8800000000000" target="_blank" rel="noreferrer" className="group rounded-3xl border border-purple-100/15 bg-purple-100/8 p-5 text-left transition hover:-translate-y-2 hover:bg-purple-100/14 sm:col-span-2"><p className="text-3xl">☏</p><h3 className="mt-4 text-xl font-black">WhatsApp</h3><p className="mt-2 text-sm text-purple-50/60">Replace 8800000000000 with your number</p></a>
              </div>
            </div>
          </div>
          ...
          <div className="glass-card scan-card rounded-[46px] p-8 md:p-12">
            ...
            <div className="mt-7 space-y-4">
              <div className="rounded-3xl border border-purple-100/12 bg-black/25 p-5"><p className="text-sm font-bold uppercase tracking-[3px] text-purple-200/70">Website Type</p><p className="mt-2 text-lg font-bold">Portfolio / Landing Page / Custom Animation</p></div>
              <div className="rounded-3xl border border-purple-100/12 bg-black/25 p-5"><p className="text-sm font-bold uppercase tracking-[3px] text-purple-200/70">Style</p><p className="mt-2 text-lg font-bold">Premium • Dark • Glowing • Responsive</p></div>
              <div className="rounded-3xl border border-purple-100/12 bg-black/25 p-5"><p className="text-sm font-bold uppercase tracking-[3px] text-purple-200/70">Built With</p><p className="mt-2 text-lg font-bold">React + Tailwind CSS + Custom Animations</p></div>
            </div>
